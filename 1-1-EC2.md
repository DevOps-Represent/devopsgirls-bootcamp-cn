...
- *安全组就像你的防火墙*。它本质上是可以访问实例IP规则列表。有一点很重要:你需要尽可能缩小远程**管理**访问范围。
...
首先, 你需要使用浏览器访问[AWS控制台](https://tw-toc.signin.aws.amazon.com/console)，使用你的账户密码登录。

![Instance Logins](./images/1-1-EC2/1-1-1-awslogin-zh.png)

### 1.B) 确保工作在正确的区域(Region)

然后，我们需要选择离我们附近的区域：点击右上角，选择**新加坡(Singapore)**

![Region](./images/1-1-EC2/1-1-0-region-zh.png)

### 2.) 进入EC2

如果单击**服务**，应该会看到类似于下面的内容。因为Amazon有很多服务，所以你可能想要在搜索框中输入*EC2*，点击之后的链接。

![DevOpsGirls-1-1-2](./images/1-1-EC2/1-1-2-gotoec2-zh.png)

### 3.) 创建密钥对

我们需要做的第一件事是创建密钥对，这是用于登录到EC2实例的密钥文件。

在左边菜单部分，单击**密钥对**菜单项，然后单击**创建密钥对**。

命名格式：`你名字拼音-DevOps-Girls`

![Create key pair](./images/1-1-EC2/1-1-3-keypair-zh.png)

点击**创建**，然后会自动下载密钥文件，请将其保存在一个合适的地方。

### 4.) 创建EC2实例

接下来我们需要做的是创建*EC2实例*。

在左边菜单部分，单击**实例**菜单项。点击**启动实例**。

![Launch instance](./images/1-1-EC2/1-1-4-launchinstance-zh.png)

### 5.) 选择AMI

AMI本质上是实例的*操作系统*。 浏览一下当前列表 - 你可以使用大量预制的AMI，包括Windows，Linux，以及其它更专业的系统。
...
![Instance details](./images/1-1-EC2/1-1-6-instancedeets-zh.png)
...
![Disks](./images/1-1-EC2/1-1-7-instancestor-zh.png)
...
在此处添加**Name**标签，命名格式：`你名字拼音-DevOps-Girls-01`
它将会显示在EC2的控制台列表上。
![Instancetags](./images/1-1-EC2/1-1-8-instancetags-zh.png)
...
另外我们需要使用SSH登录到实例上面，并且为了安全起见，需要在SSH规则**来源**中选择 **我的IP**，确保只有你可以访问实例的管理流量。

![Secgroups](./images/1-1-EC2/1-1-9-secgroups-zh.png)

单击**审核和启动**，在这里你将能看到实例的所有配置信息
![Review](./images/1-1-EC2/1-1-9-review-zh.png)

然后单击**启动**

### 11.) 使用之前创建的密钥对

最后，我们终于到了选择要访问实例的密钥文件的部分。 还记得你之前创建的钥匙对吗？ 选择**选择现有密钥对**，并选择之前创建的密钥对的名称, 单击**启动实例**。

![Keypair](./images/1-1-EC2/1-1-10-keypair-zh.png)

### 12.) 启动实例

实例启动后，你将看到一个显示实例ID的窗口，单击实例ID的链接。

![Launching](./images/1-1-EC2/1-1-11-launch-zh.png)

### 13.) 查看实例属性

如果单击实例ID，你就可以在EC2实例列表中看到它。 在底部窗口，你将看到一个实例属性列表： 它的名称是什么，IP地址是什么，以及它在哪里。你需要找到它的**公有IP**的属性。
![Properties](./images/1-1-EC2/1-1-12-properties-zh.png)
...
- [Mac](8-1-SSH-from-Mac.md)

- [Windows](8-2-SSH-from-Windows.md)
...
![Image](./images/1-1-EC2/1-1-13-blankbrowser.png)
...
![Image](./images/1-1-EC2/1-1-14-mysql.png)

### 21.) 再次尝试使用浏览器访问实例

现在服务已经启动，你将可以看到类似安装页面的内容：

![Image](./images/1-1-EC2/1-1-15-wordpresssetup.png)
...
![Image](./images/1-1-EC2/1-1-16-wpsqlsetup.png)

如果一切顺利，你将拥有一套完整的Wordpress服务！

![Image](./images/1-1-EC2/1-1-17-wpfinished.png)

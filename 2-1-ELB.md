...
在选择负载均衡器类型的页面，你需要选择`应用程序负载均衡器`。

![Image](./images/2-1-ELB/2-1-2-alb-zh.png)

### 3.) 定义负载均衡器

- 在 `名称` 处写下你的负载均衡器的名字，格式：*myname*-alb，
- 在 `模式` 处选择 *面向 Internet*
- IP 地址类型: *IPV4*

![Image](./images/2-1-ELB/2-1-3-albname-zh.png)
...
负载均衡器仅将流量路由至这些可用区中的目标。每个可用区您只能指定一个子网。必须从至少两个可用区中指定子网以提高负载均衡器的可用性。

![Image](./images/2-1-ELB/2-1-5-az-zh.png)

### 6.) 为负载均衡器创建安全组

点击`下一步: 配置安全设置`，选择`创建一个新安全组`选项。与之前为实例创建的安全组类似，相当于设置允许哪种类型的流量通过的防火墙。

![Image](./images/2-1-ELB/2-1-6-secgroups-zh.png)

除了"来源"选择"我的IP"，其他的配置都保持默认值。

### 7.) 配置路由

点击`下一步: 配置路由`，选择创建`新的目标组`，填入名称，其他的配置保持不变

![Image](./images/2-1-ELB/2-1-7-route-zh.png)

现在，我们选择`TCP`协议，`80`端口。换句话说，负载均衡器会检查所有实例的80端口，如果它没有响应错误就会被标记为"健康"。

### 8.) 向你的目标组注册目标

在实例列表中上，我们将选择你在上一个模块中创建的实例，将其添加至已注册。 ，只要注册过程完成且此目标通过初始运行状况检查，负载均衡器就会开始将请求路由至目标。

![Image](./images/2-1-ELB/2-1-8-target-zh.png)
...
![Image](./images/2-1-ELB/2-1-9-review-zh.png)
...
![Image](./images/2-1-ELB/2-1-10-details-zh.png)
